...
serial_port = 'COM3'
baud_rate = 115200
save_directory = r"C:\Users\Bazar\Desktop\My-ESP32_Projects\ESP32_CAM\ESP32-CAM_SendImagesOverSerial\captured_images"  # <---- ------ Folder to save images ------ ---->
...
while True:
    ...
    if len(img_data) == img_len:
        ...
        try:
            ...
            plt.pause(2)           # keep window open for 2 sec
            plt.close()

            print(f"<---- Saved and displayed {filename} ---->")
        except Exception as e:
            print(f"<---- Warning: Saved {filename} but could not display: {e} ---->")
